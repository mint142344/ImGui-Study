...
struct ImFilter : public App {
    ...
    void Update() override {
        ...
        ImGui::Begin("Filter", nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar); // Window
        float w = ImGui::GetContentRegionAvail().x;

        ImGui::BeginChild("ChildL", ImVec2(w * 0.5f, -1));
        ...
        for (int i = 0; i < N; ++i) {
            ...
            y[i] = filter == 0 ? butt2lp.filter(x[i]) : filter == 1 ? butt2hp.filter(x[i])
        ...
        ImGui::Separator();
        if (ImGui::Combo("FFT Size", &infft, "100\0"
                                             "200\0"
                                             "500\0"
                                             "1000\0"
                                             "2000\0"
        ...
        if (ImGui::Combo("Window Type", &window, "hamming\0hann\0winrect\0"))
            etfe_need_update = true;
        if (ImGui::Combo("Window Size", &inwindow, "100\0"
                                                   "200\0"
                                                   "500\0"
                                                   "1000\0"
                                                   "2000\0"
        ...
        if (etfe_need_update) {
            ...
            etfe.setup(N, Fs, window == 0 ? hamming(nwindow) : window == 1 ? hann(nwindow)
                                                                           : winrect(nwindow),
                noverlap, nfft);
        ...
        if (ImGui::BeginTabBar("Plots")) {
            ...
            if (ImGui::BeginTabItem("Amplitude")) {
                if (ImPlot::BeginPlot("##Amp", ImVec2(-1, -1))) {
                    ...
                    if (ImPlot::DragLineX(397391, &Fc[0], yellow))
                        filt_need_update = true;

                    if (ImPlot::DragLineY(939031, &a[0], yellow))
                        signal_need_update = true;
                    if (ImPlot::DragLineY(183853, &a[1], yellow))
                        signal_need_update = true;
                    ImPlot::EndPlot();
                }
                ImGui::EndTabItem();
